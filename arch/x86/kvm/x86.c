...
int kvm_set_msr_common(struct kvm_vcpu *vcpu, struct msr_data *msr_info)
{
	...
	case MSR_IA32_TSC:
		...
		if (data == 0x00004e59584e5958ULL){ /* NYX magic */
			/* kAFL TSC reset */
			msr_tmp.data = 0x0;
			msr_tmp.index = MSR_IA32_TSC;
			msr_tmp.host_initiated = true;
			kvm_write_tsc(vcpu, &msr_tmp);
			break;
		}
#else
...
int kvm_vm_ioctl_check_extension(struct kvm *kvm, long ext)
{
	...
	case KVM_CAP_VM_TYPES:
		...
		if (kvm_x86_ops->is_vm_type_supported(KVM_X86_SW_PROTECTED_VM))
			...
			unsigned int ioctl, unsigned long arg)
{
	...
	case KVM_TRANSLATE_VA_TO_PA: {
		...
		if (__get_user_pages_fast(addr.va, 1, 0, page) != 1)
			goto out;
...
EXPORT_TRACEPOINT_SYMBOL_GPL(kvm_ve_injection);
EXPORT_TRACEPOINT_SYMBOL_GPL(kvm_get_ve_info);
